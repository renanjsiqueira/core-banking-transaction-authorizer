 ...
 *
 * <p>Runs inside a single ACID transaction. Concurrency on the same account is
 * serialized by reading the account with a pessimistic write lock
 * ({@link AccountRepository#findByIdForUpdate}); different accounts proceed in
 * parallel. Idempotency is keyed by {@code transactionId}: a replay with the
 * same logical payload returns the stored result without re-applying the
 * balance, while a replay with a different payload is a conflict.
 */
@Service
public class TransactionAuthorizationServiceImpl implements TransactionAuthorizationService {
    ...
    public TransactionResponse authorize(UUID transactionId, TransactionRequest request) {
        ...
        BigDecimal amount = normalize(request.amount().value());
    ...
    private boolean isSameLogicalPayload(TransactionEntity transaction, TransactionRequest request) {
        return transaction.getAccountId().equals(request.accountId())
                ...
                && transaction.getAmount().compareTo(normalize(request.amount().value())) == 0
    ...
    private TransactionEntity authorizeCredit(UUID transactionId,
                                              ...
                                              BigDecimal amount) {
        BigDecimal newBalance = normalize(account.getBalanceAmount().add(amount));
    ...
    private TransactionEntity authorizeDebit(UUID transactionId,
                                             ...
                                             AccountEntity account,
                                             BigDecimal amount) {
        BigDecimal newBalance = normalize(account.getBalanceAmount().subtract(amount));
